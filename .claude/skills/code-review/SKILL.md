...
paths:
  - "src/**/*.{ts,js,py,go,java,cs,rb,rs,cpp,c,php}"
  - "**/*.{ts,js,py,go,java,cs}"
...
- **No marcar todo como crítico** — Reservar 🔴 solo para vulnerabilidades reales o bugs con impacto demostrable. Inflar severidades desacredita el review.
- **No reescribir código funcional** — Code review analiza, no refactoriza. Si algo funciona pero podría mejorar, usar 🔵 Sugerencia.
- **Falsos positivos de seguridad** — No toda concatenación de strings es XSS; verificar si el output es sanitizado antes o después. Confirmar el contexto real antes de marcar.
- **Ignorar el contexto del proyecto** — Un `console.log` en producción es crítico; en un CLI dev-only puede ser intencional. Leer CLAUDE.md del módulo primero.
- **Reviews parciales sin decirlo** — Si solo revisas parte del código, explicitarlo en el header. No dar un `✅ Aprobado` global sobre una revisión parcial.
